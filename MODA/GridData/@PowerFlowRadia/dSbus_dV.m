function [ dSbus_dVm, dSbus_dVa ] = dSbus_dV( obj, V )
...
if issparse(obj.Ybus)           %% sparse version (if Ybus is sparse)
    diagV       = sparse(1:obj.nb, 1:obj.nb, V, obj.nb, obj.nb);
    diagIbus    = sparse(1:obj.nb, 1:obj.nb, Ibus, obj.nb, obj.nb);
    diagVnorm   = sparse(1:obj.nb, 1:obj.nb, V./abs(V), obj.nb, obj.nb);
else                        %% dense version
    diagV       = diag(V);
    diagIbus    = diag(Ibus);
...
dSbus_dVm = diagV * conj(obj.Ybus * diagVnorm) + conj(diagIbus) * diagVnorm;
dSbus_dVa = 1j * diagV * conj(diagIbus - obj.Ybus * diagV);
end
